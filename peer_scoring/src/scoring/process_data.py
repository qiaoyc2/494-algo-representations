...
def tsv_parser(
  ...
  try:
    ...
    # When header=True, skip the file's first line and use our column names so casing is correct
    if header:
      lines = rawTSV.split("\n")
    ...
    if useCols and chunkSize:
      ...
      # Avoid "dtype of categories must be the same" on concat: cast category cols to object
...
def _mapping_for_read(mapping) -> Dict:
  """Use object instead of category for reading to avoid pandas union_categoricals on large files."""
...
def tsv_reader(
  ...
  if os.path.isdir(path):
    ...
    # Avoid "dtype of categories must be the same" when concat: cast category cols to object first
...
def read_from_tsv(
  ...
  if ratingsPath is None:
    ...
    # Normalize rater column name: file may have RaterParticipantId or other casing
    if c.raterParticipantIdKey not in ratings.columns:
      rater_col = next(
        (col for col in ratings.columns if col.lower().replace("_", "") == "raterparticipantid"),
        None,
      )
      if rater_col is not None:
        ratings = ratings.rename(columns={rater_col: c.raterParticipantIdKey})
    # Reorder to match expected columns (e.g. after rename)
    if list(ratings.columns) != list(c.ratingTSVColumns):
      ratings = ratings[c.ratingTSVColumns]
