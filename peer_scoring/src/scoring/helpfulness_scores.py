...
def compute_general_helpfulness_scores(
  ...
  helpfulnessScores = authorCounts.join(
    raterCounts,
    how="outer",
    lsuffix="_author",
    rsuffix="_rater",
  ...
  # Pandas versions differ on reset_index() output name ("index", author key, or rater key).
  if c.raterParticipantIdKey not in helpfulnessScores.columns:
    id_candidates = [c.defaultIndexKey, c.noteAuthorParticipantIdKey, "level_0"]
    for col in id_candidates:
      if col in helpfulnessScores.columns:
        helpfulnessScores = helpfulnessScores.rename(columns={col: c.raterParticipantIdKey})
        break
