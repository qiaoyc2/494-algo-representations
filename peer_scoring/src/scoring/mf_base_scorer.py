    ...
    if not self._useReputation:
      ...
      with c.time_block("Final round MF"):
        ...
        if len(finalRoundRatings) == 0:
          logger.warning(
            "No ratings remain after helpfulness filtering; skipping final-round MF and "
            "falling back to first-round MF parameters."
          )
          noteParams = noteParamsUnfiltered
          raterParams = raterParamsUnfiltered
          # Keep downstream diligence step working with non-empty ratings.
          finalRoundRatings = ratingsForTraining[
            [
              c.noteIdKey,
              c.raterParticipantIdKey,
