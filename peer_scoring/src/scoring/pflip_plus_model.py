...
class PFlipPlusModel(object):
  ...
  def _compute_rater_factors(self, prescoringRaterModelOutput: pd.DataFrame) -> pd.DataFrame:
    ...
    """
     # Grab only the Core factors (since Expansion scorers are disabled)
    raterFactors = prescoringRaterModelOutput[
      prescoringRaterModelOutput[c.scorerNameKey] == Scorers.MFCoreScorer.name
    ][[c.raterParticipantIdKey, c.internalRaterFactor1Key]]

    # Rename the column to 'RATER_FACTOR' so the rest of the model can understand it
    raterFactors = raterFactors.rename(
      columns={c.internalRaterFactor1Key: _RATER_FACTOR}
    )
