...
class MatrixFactorization:
  ...
  def run_mf(
    ...
    note_count = self.mf_model.note_factors.weight.data.shape[0]
    user_count = self.mf_model.user_factors.weight.data.shape[0]
    ratings_per_note = (len(ratings) / note_count) if note_count else 0.0
    ratings_per_user = (len(ratings) / user_count) if user_count else 0.0
    logger.info(f"Ratings per note in dataset: {ratings_per_note}")
    logger.info(f"Ratings per user in dataset: {ratings_per_user}")
